...
>
  ...
  {#if showLegend}
    <ul class="proportion-bar-legend" aria-label="Segment breakdown" data-pw={typeof testId === 'string' ? `${testId}-legend` : null}>
      {#each computedSegments as computedSegment, legendIndex (legendIndex)}
        <li class="proportion-bar-legend-item" data-pw={typeof testId === 'string' ? `${testId}-legend-item-${legendIndex}` : null}>
          <span
