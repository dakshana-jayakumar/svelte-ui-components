...
>
    ...
    <div class="statcard-header" data-pw={typeof testId === 'string' ? `${testId}-header` : null}>
      <div class="statcard-header-left">
        {#if hasTitle}
          ...
          {:else}
            <div class="statcard-title" data-pw={typeof testId === 'string' ? `${testId}-title` : null}>{title}</div>
          {/if}
        {/if}
        {#if checkbox}
  ...
  {#if hasRows && rows}
    <div class="statcard-rows" class:statcard-rows-horizontal={rowsDirection === 'row'} data-pw={typeof testId === 'string' ? `${testId}-rows` : null}>
      {#each rows as row, rowIndex (rowIndex)}
        {#if rowIndex > 0}
          <div class="statcard-row-divider" data-pw={typeof testId === 'string' ? `${testId}-row-divider-${rowIndex}` : null}></div>
        {/if}
        <div class="statcard-row" data-pw={typeof row.testId === 'string' ? row.testId : null}>
          ...
          <div class="statcard-row-value-line">
            <div class="statcard-value" data-pw={typeof testId === 'string' ? `${testId}-value-${rowIndex}` : null}>{row.value}</div>
            {#if typeof row.change === 'number'}
              <DeltaIndicator value={row.change} invertColors={row.invertChangeColors ?? false} testId={typeof testId === 'string' ? `${testId}-delta-${rowIndex}` : undefined} />
            {/if}
          ...
          {#if Array.isArray(row.breakdown) && row.breakdown.length > 0}
            ...
            <div class="statcard-breakdown-grid">
              {#each row.breakdown as breakdownItem, breakdownIndex (breakdownIndex)}
                <div class="statcard-breakdown-item" data-pw={typeof testId === 'string' ? `${testId}-breakdown-item-${rowIndex}-${breakdownIndex}` : null}>
  ...
  {:else}
    <div class="statcard-value-row">
      {#if typeof valueSnippet === 'function'}
        <div class="statcard-value" data-pw={typeof testId === 'string' ? `${testId}-value` : null}>{@render valueSnippet()}</div>
      {:else if typeof value === 'string' && value.length > 0}
        <div class="statcard-value" data-pw={typeof testId === 'string' ? `${testId}-value` : null}>{value}</div>
      {/if}
  ...
  {#if typeof subtitle === 'string' && subtitle.length > 0}
    <div class="statcard-subtitle" data-pw={typeof testId === 'string' ? `${testId}-subtitle` : null}>{subtitle}</div>
  {/if}
